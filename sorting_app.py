...
class sorting_App:
    ...
    #==========Section-4==========#
        ...
        self.progress.place(x=55, y=575,height=18,width=1240)

        self.lbl_last_sorted = Label(
        self.root,
        text="Last Sorted: Not Yet",
        font=("times new roman", 12),
        bg="white",
        fg="gray"
    )
        self.lbl_last_sorted.place(x=50, y=650)
    ...
    def start_function(self):
        if self.var_foldername.get() != "":
            ...
            for i in self.all_files:
                ...
                if os.path.isfile(file_path):
                    ...
                    # 🔥 PROGRESS BAR UPDATE
                    self.progress['value'] = (c / total) * 100
                    self.root.update_idletasks()

                    # status labels
                    self.lbl_st_total.config(text="TOTAL:" + str(total))
                    self.lbl_st_moved.config(text="MOVED:" + str(c))
                    self.lbl_st_left.config(text="LEFT:" + str(total - c))
        ...
        self.lbl_last_sorted.config(
        text="Last Sorted: " + datetime.now().strftime("%d-%m-%Y %I:%M:%S %p")
        )
